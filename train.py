...
# build model
model = model.build(width=img_dims[0], height=img_dims[1], depth=img_dims[2], classes=2)
...
for epoch in range(epochs):
    for i in range(len(trainX) // batch_size):
        X_batch, y_batch = aug.flow(trainX[i * batch_size:(i + 1) * batch_size], trainY[i * batch_size:(i + 1) * batch_size]).__next__()
